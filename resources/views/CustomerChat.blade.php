...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    ...
    <div class="bg-white shadow-lg rounded-lg w-full max-w-3xl flex flex-col h-[80vh]">
        ...
        {{-- Form --}}
        <form action="{{ route('customer.chat.send', ['chatId' => $chat->id]) }}" method="POST"
    ...
    <script>
        ...
        messagesEl.scrollTop = messagesEl.scrollHeight; // Scroll to latest on load
        ...
        window.Echo.private(`chat.{{ $chat->id }}`)
            .listen('MessageSent', (e) => {
                ...
                div.innerHTML = `
                <div class="${isMine ? 'bg-blue-500 text-white' : 'bg-white text-gray-900 border'} px-4 py-2 rounded-lg max-w-xs shadow">
                    ${e.message.content}
