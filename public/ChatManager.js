class ChatManager {
    ...
    init() {
        ...
        this.currentUserId = document
            .querySelector('meta[name="user-id"]')
            ?.getAttribute("content");
        this.userType = document
    ...
    handleNotification(notificationData) {
        ...
        if (
            notificationData.chat_id != this.currentChatId ||
            !document.hasFocus()
        ) {
    ...
    createMessageElement(messageData) {
        ...
        switch (messageData.type) {
            case "text":
                messageContent = `<p>${this.escapeHtml(
                    messageData.content
                )}</p>`;
                break;
            case "image":
            ...
            case "file":
                ...
                }" class="underline text-blue-200 hover:text-blue-300" target="_blank">📄 ${this.escapeHtml(
                    messageData.content || "Download File"
                )}</a>`;
                break;
            case "voice":
    ...
    showNotificationToast(notificationData) {
        ...
        toast.className =
            "fixed top-4 right-4 bg-white border-l-4 border-blue-500 rounded-lg shadow-lg p-4 max-w-sm z-50";
        toast.style.animation = "slideInRight 0.3s ease-out";

        toast.innerHTML = `
            <div class="flex justify-between items-start">
                <div>
                    <h6 class="font-semibold text-sm text-gray-800">${this.escapeHtml(
                        notificationData.title
                    )}</h6>
                    <p class="text-sm text-gray-600 mt-1">${this.escapeHtml(
                        notificationData.message
                    )}</p>
                    <p class="text-xs text-gray-500 mt-1">From: ${this.escapeHtml(
